...
public class SoundEntity {
    ...
    public SoundEntity(String name, String type, String url) {
        this.name = name;
        this.type = type;
        this.url = url;
    }

    public SoundEntity(String name, String type, String url, Ringtone ringtone) {
        this(name, type, url);
     ...
     */
    @Nullable
    public static SoundEntity fromString(String string) {
        if (string.contains(SEPARATOR)) {
            String[] data = string.split(SEPARATOR);
            if (data.length == 3
                    && data[0].length() > 0 && data[1].length() > 0 && data[2].length() > 0)
                return new SoundEntity(data[0], data[1], data[2]);
        }
    ...
    public boolean equals(Object obj) {
        return (obj != null && obj instanceof SoundEntity && ((SoundEntity) obj).url.equals(url));
    }
}
